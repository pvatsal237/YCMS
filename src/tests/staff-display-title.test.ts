...
describe("staffDisplayTitle", () => {
  it("shows kitchen lead instead of volunteer when the person leads a department", () => {
    expect(staffDisplayTitle("ATTENDANCE_VOLUNTEER", ["KITCHEN"])).toBe("Kitchen Department Lead");
    expect(staffDisplayTitle("ATTENDANCE_VOLUNTEER", ["TRANSPORTATION"])).toBe("Transportation Department Lead");
